...
@Repository
public class MemberShipManagementDaoImpl implements MemberShipManagementDao {
	...
	public Double accountBalanceOf(final Long userId) {
		return jdbctemplate.queryForObject(
				"SELECT AVAILABLE_BALANCE FROM TRANSPORT_MANAGEMENT.MEMBERSHIP WHERE USER_ID = ?",
	...
	public boolean deductFromAccount(final Long userid, final Double amount) {
		return this.jdbctemplate.update(
				"UPDATE TRANSPORT_MANAGEMENT.MEMBERSHIP SET AVAILABLE_BALANCE = ? WHERE USER_ID = ?",
	...
	public boolean changeCardStatus(Long userId, CardStatus cardStatus) {
		return this.jdbctemplate.update("UPDATE TRANSPORT_MANAGEMENT.MEMBERSHIP SET CARD_STATUS = ? WHERE USER_ID = ?",
				new PreparedStatementSetter() {
					@Override
					public void setValues(PreparedStatement preparedStatement) throws SQLException {
